...
def component_reduction(X_df, y_df, model, max_components=50, pca=True):
    """
    Use PCA or PLS to return list with average RMSE
    for all number of components tested
    ...
    if pca:
        ...
        # Iterate over principle components for avg RMSE
        for i in np.arange(1, max_components+1):
            # Resets model
            curr_model = model
            _, rmse = kfold_validate_score(
                X_reduced[:, :i], y_df, model=curr_model
            )
    ...
    # Fit for PLS
        ...
        for i in np.arange(1, max_components+1):
            ...
            _, rmse = kfold_validate_score(
                X_train_pls, y_df, model=curr_model
            )
...
def pls_comp_list(df, comp_num, threshold=0.10):
    """
    Returns a df of the most important features of a game
    by PLS component X_weights

    :param df: dataframe object, dataframe of X_weights generated
                                 by PLS Regression
    :param comp_num: int, chosen PLS component
    :param threshold: float, threshold of X_weight to include in output df

    :return: dataframe object, dataframe of the most important features
                               by weight of a chosen PLS Component
    """
